...
def consume_prefix_in_state_dict_if_present(state_dict, prefix):
    ...
    ..note::
        Given a `state_dict` from a DP/DDP backbone, a local backbone can load it by applying
        `consume_prefix_in_state_dict_if_present(state_dict, "module.")` before calling
        :meth:`torch.nn.Module.load_state_dict`.
    Args:
        state_dict (OrderedDict): a state-dict to be loaded to the backbone.
        prefix (str): prefix.
    """
    ...
    if "_metadata" in state_dict:
        ...
        for key in list(metadata.keys()):
            ...
            # 'module': for the actual backbone.
